...
if (!class_exists('SjVmSearchProBaseHelper')) {
    /**
     * BaseHelper for com_content only.
     */
    abstract class SjVmSearchProBaseHelper
    {
        /**
            ...
            if (!isset(self::$image_cache[$hash][$item->id])) {
                        ...
                        switch ($type) {
                            case 'description':
                                ...
                                for ($i = 0; $i < count($inline_images); $i++) {
                                    $inline_images[$i]['title'] = $item->product_name;
                                    $inline_images[$i]['alt'] = $item->product_name;
                            ...
                            case 'external':
                                ...
                                for ($i = 0; $i < count($files); $i++) {
                                    if (file_exists($files[$i]) || @GetImageSize($files[$i])) {
                                        array_push($images_path, array('src' => $files[$i], 'title' => $item->product_name, 'alt' => $item->product_name));
                                    }
                                }
                                break;
                            default:
                ...
                if (count($images_path) == 0 && $params->get($prefix . '_placeholder', 1) == 1) {
                    if (strpos($params->get($prefix . '_placeholder_path', null), 'nophoto') === false) {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_extraslider/assets/images/nophoto.png'), 'title' => $item->product_name, 'alt' => $item->product_name, 'class' => 'respl-placeholder');
                    } else {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', 'modules/mod_sj_vm_extraslider/assets/images/nophoto.png'), 'title' => $item->product_name, 'alt' => $item->product_name, 'class' => 'placeholder');
                    }
                }
